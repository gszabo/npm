...
function list (args) {
  const conf = config()
  return profile.listTokens(conf).then(tokens => {
    if (conf.json) {
...
function rm (args) {
  ...
  return profile.listTokens(conf).then(tokens => {
    ...
    return Bluebird.map(toRemove, key => profile.removeToken(key, conf))
  }).then(() => {
    if (conf.json) {
